...
Reply with the *number* (1-5) of your group type:`
  ...
  async handleGroupTypeSelection(senderId, messageText) {
    ...
    const groupTypes = ['company_official', 'building_society', 'sports_group', 'close_friends', 'general']
...
Current selection: ${this.pendingConfigurations.get(senderId).warningLocation === 'group' ? 'Group warnings' : 'Private warnings'}`
  ...
  async handleWarningCountSelection(senderId, messageText) {
    ...
    if (isNaN(warningCount) || warningCount < 1 || warningCount > 10) {
      await this.sock.sendMessage(senderId, { 
        text: "Please enter a valid number between 1 and 10 for the warning limit." 
      })
      return
    }
...
💬 **Warning Location:** ${config.warningLocation === 'group' ? 'Group Chat' : 'Private Messages'}
...
If you've already configured me, just add me to your group and I'll start working automatically! 🤖`
  ...
  async handleGroupJoin(groupId, senderId, messageText) {
    ...
    try {
      ...
      const isAddedMessage = messageText.includes('added') && messageText.includes(botNumber)
  ...
  async isNewlyAddedToGroup(groupId) {
    try {
      const existingConfig = await this.db.collection('group_configurations').findOne({ groupId })
  ...
  async handleBotAddedToGroup(groupId, addedBy, groupMetadata) {
    try {
      ...
      const userConfig = await this.db.collection('user_configurations')
        .findOne({ userId: addedBy })

      if (!userConfig) {
        const noConfigMessage = `❌ *Configuration Required*

Hello! I was added to this group, but the person who added me (@${addedBy.split('@')[0]}) hasn't configured me yet.
...
✅ ${config.adminStatus ? 'Remove users after multiple violations' : 'Alert admins about repeated violations'}
✅ Send warnings ${config.warningLocation === 'group' ? 'in the group' : 'privately'}
...
• ${config.warningLocation === 'group' ? 'Warnings are sent publicly in the group' : 'Warnings are sent privately'}
• ${config.adminStatus ? 'After maximum warnings, users are removed' : 'After maximum warnings, admins are notified'}
...
_I'm now active and monitoring. Play nice, everyone!_ 😊`
  ...
  async handleUserRemoved(groupId, userId) {
    try {
      ...
      const groupAdmin = await this.db.collection('group_admins')
...
Please remember to follow our group guidelines to maintain a respectful environment.`
  ...
  async handleGroupModeration(groupId, senderId, messageText, msg) {
    try {
      ...
      const groupConfig = await this.db.collection('group_configurations')
      ...
      const isViolation = await this.analyzeMessage(groupId, senderId, messageText, groupConfig.groupType)
...
`
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a content moderation assistant. Be strict but fair in your analysis based on the group type.'
          },
          {
            role: 'user',
            content: prompt
  ...
  async handleViolation(groupId, senderId, messageText, messageKey, groupConfig) {
    try {
      ...
      if (lastWarning && now - lastWarning < config.bot.warningCooldown * 1000) {
        return
      }
      ...
      if (groupConfig.warningLocation === 'private') {
        await this.sendPrivateWarning(senderId, warningCount, messageText, groupConfig)
      } else {
        await this.sendGroupWarning(groupId, senderId, warningCount, messageText, messageKey, groupConfig)
      }
  ...
  async sendPrivateWarning(senderId, warningCount, violatingMessage, groupConfig) {
    try {
      ...
      if (remainingWarnings > 0) {
        warningMessage = `⚠️ *Group Policy Warning ${warningCount}/${groupConfig.warningLimit}*

Hello @${userNumber},

Your recent message in the ${groupTypeName} violated our group guidelines.

*Violated Message:* "${violatingMessage.length > 100 ? violatingMessage.substring(0, 100) + '...' : violatingMessage}"

*Remaining Warnings:* ${remainingWarnings}

*Group Rules:*
${GROUP_POLICIES[groupConfig.groupType].rules.map((rule, index) => `${index + 1}. ${rule}`).join('\n')}

Please maintain respectful communication. ${groupConfig.adminStatus ? 'Further violations may result in removal from the group.' : 'Please follow the group guidelines.'}`
      } else {
        warningMessage = `🚫 *Final Warning - ${groupConfig.warningLimit}/${groupConfig.warningLimit}*

@${userNumber}, you have reached the maximum number of warnings for the ${groupTypeName}.

${groupConfig.adminStatus ? 'You will be removed from the group for repeated violations.' : 'Please follow group guidelines to avoid further issues.'}`
      }
      ...
      console.log(`Private warning sent to ${userNumber}. Count: ${warningCount}`)
    } catch (error) {
      console.error('Error sending private warning:', error)
    }
  }

  async sendGroupWarning(groupId, senderId, warningCount, violatingMessage, quotedMessage, groupConfig) {
    try {
      const userNumber = senderId.split('@')[0]
      const remainingWarnings = groupConfig.warningLimit - warningCount
      ...
      if (remainingWarnings > 0) {
        warningMessage = `⚠️ *Warning ${warningCount}/${groupConfig.warningLimit}*
...
@${userNumber}, you have reached the maximum number of warnings (${groupConfig.warningLimit}). ${groupConfig.adminStatus ? 'You will be removed from the group for repeated violations.' : 'Please follow group guidelines.'}`
      }
      ...
      console.log(`Group warning sent to ${userNumber} in group ${groupId}. Count: ${warningCount}`)
    } catch (error) {
...
*Warning limit reached:* ${groupConfig.warningLimit}/${groupConfig.warningLimit}`
  ...
  async updateWarningCount(groupId, senderId) {
    try {
      ...
      return result.value ? result.value.count : 1
    } catch (error) {
